...
def generate_pft_netcdf(topo, latitudes, pft_values):
    ...
    for i in range(len(latitudes)):
        ...
        for pft_layer in numpy.argwhere(pft_values[i] > 0):
            # Set one hemisphere
            arr[pft_layer, _get_lat_index(_to):_get_lat_index(_from), :] = pft_values[i, pft_layer]
            # Set other hemisphere
            arr[pft_layer, nb_lat-_get_lat_index(_from):nb_lat-_get_lat_index(_to), :] = pft_values[i, pft_layer]
        # Go to next latitude
        _from = _to
    ...
    ds = xr.Dataset(
        ...
        data_vars = {
            "maxvegetfrac": (["time_counter", "veget", "lat", "lon"], arr[numpy.newaxis, ...])
        }
    )
    ds.lat.attrs = {'bounds': 'bounds_lat', 'valid_max': 90.0, 'long_name': 'Latitude', 'valid_min': -90.0, 'units': 'degrees_north', 'axis': 'Y'}
    ds.lon.attrs = {'bounds': 'bounds_lon', 'valid_max': 180.0, 'long_name': 'Longitude', 'valid_min': -180.0, 'axis': 'X', 'units': 'degrees_east', 'modulo': 360.0, 'topology': 'circular'}
    ds.time_counter.attrs = {'units': 'years since 0-1-1', 'calendar': 'gregorian', 'axis': 'T'}
    ds.veget.attrs = {'valid_max': 13, 'long_name': 'Vegetation Classes', 'valid_min': 1, 'units': '-'}
