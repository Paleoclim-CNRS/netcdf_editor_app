...
def _migrate_front_across_flats(front, arrays, trip_values, iterations, iteration):
    ...
    next_cells[:, 1] = numpy.where(next_cells[:, 1] == -1, iterations.shape[1] - 1, next_cells[:, 1])
    ...
    iteration += 1
    return _migrate_front_across_flats(next_cells[:, :2].T, arrays, trip_values, iterations, iteration)
...
def _add_gradient_to_flats(topo):
    ...
    # Add the values to iterations this is so the algo knows that these cells ahev already been seen and will expand / migrate from here
...
def _calculate_dx(rlat):
    rEarth = 6370
    return (2 * numpy.pi * rEarth * numpy.cos(rlat * numpy.pi / 180)) / 360.
...
def get_next_cell(cells, temp_array, trip):
    ...
    # Handle periodicity
    if ( jp1 < 0 ): jp1 = temp_array.shape[1] -1
    if ( jp1 >= temp_array.shape[1] ): jp1 = 0
    next_cell = (ip1, jp1)
...
def calculate_outflow_points(basins, trip):
    ...
    outflow_points = []
    for basin_nb in range(numpy.nanmin(basins).astype(int), numpy.nanmax(basins).astype(int) + 1):
        nboutflow = 0
        ...
        for k in range(len(x)):
            ...
            # Handle periodicity
            if ( jp1 < 0 ): jp1 = basins.shape[1] -1
            if ( jp1 >= basins.shape[1] ): jp1 = 0
            next_cell = (ip1, jp1)
        ...
        if nboutflow > 1:
            raise AssertionError("error occured to many outflow points for basin {}".format(basin_nb))
...
def calculate_distbox(flength, trip):
    ...
    distbox = diffs[numpy.where(~numpy.isnan(trip), trip - 1, 0).astype(int), xx, yy] * 1000
    return distbox
...
def write_to_netcdf(filename, topo, trip, basins, topo_index, dzz, distbox, orog, flength, rlat, rlon):
    ds_final = xr.Dataset(
        coords = {
            'nav_lon': (['y', 'x'], rlon),
            'nav_lat': (['y', 'x'], rlat)
...
def run_routines(topo, latitudes):
    ...
    write_to_netcdf("test.nc", topo, trip, basins, topo_index, dzz, distbox, orog, river_length, rlat, rlon)
