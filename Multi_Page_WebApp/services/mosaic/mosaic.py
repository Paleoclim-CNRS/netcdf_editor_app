...
class MosaicRunner(object):
    ...
    @property
    def domsk_dir(self):
        return os.path.join(self.root, "ORCA2_BLD", "DOMSK", self.OceMdl + "." + self.simulation_name)

    @property
    def mosaic_dir(self):
        return os.path.join(self.root, "ORCA2_BLD", self.OceMdl + "." + self.simulation_name + "x" + self.AtmMdl)
    ...
    def run_file_generation(self):
        ...
        # Run Grids.sh -r
        print("Running Grids", flush=True)
        subprocess.Popen(['./grids.sh', '-r'], cwd=mozaic_dir, stdout=subprocess.PIPE).wait()     
...
if __name__ == '__main__' :
    ...
    r.setup_domsk('/tmp/data/bathyORCA2.RupelianTotalV1.nc', '/tmp/data/coordinates_paleorca2_40Ma.nc')
    r.run_domsk()
