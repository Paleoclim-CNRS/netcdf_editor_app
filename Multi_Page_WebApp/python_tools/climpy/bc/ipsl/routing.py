...
def run_routines(topo, latitudes, custom_orca=None):
    print(f"{datetime.now()} Fixing Topo", flush=True)
    topo = fix_topo(topo, latitudes)
    print(f"{datetime.now()} Calculating orogen", flush=True)
    orog = calculate_orog(topo)
    print(f"{datetime.now()} Calculating Ocean Mask", flush=True)
    omsk = calculate_omsk(topo)
    print(f"{datetime.now()} Calculating runoff directions", flush=True)
    trip = calculate_trip(topo, omsk)
    print(f"{datetime.now()} Calculating curvilinear coords", flush=True)
    rlon, rlat = calculate_curvilinear_coordinates()
    print(f"{datetime.now()} Calculating grid area", flush=True)
    area = calculate_area(rlat)
    print(f"{datetime.now()} Calculating Runoff basins", flush=True)
    basins = calculate_basins(topo, trip, area)
    print(f"{datetime.now()} Calculating distance to ocean", flush=True)
    ocean_distances = calculate_ocean_distances(trip, rlat)
    print(f"{datetime.now()} Calculating length of rivers", flush=True)
    river_length = calculate_river_lengths(topo, trip, ocean_distances, omsk)
    print(f"{datetime.now()} Calculating Distbox", flush=True)
    distbox = calculate_distbox(river_length, trip)
    print(f"{datetime.now()} Calculating outflow points", flush=True)
    outflow_points = calculate_outflow_points(basins, trip)
    print(f"{datetime.now()} Calculating Trip Values", flush=True)
    trip = calculate_trip_outflow_values(trip, outflow_points, basins, omsk, rlat)
    print(f"{datetime.now()} Calculating dzz", flush=True)
    dzz = calculate_dzz(topo, trip, distbox, omsk)
    print(f"{datetime.now()} Calculating topo_index", flush=True)
    topo_index = calculate_topo_index(distbox, dzz, omsk)

    print(f"{datetime.now()} Creating routing file", flush=True)
    ds_routing = create_routing_netcdf(
        topo, trip, basins, topo_index, dzz, distbox, orog, river_length, rlat, rlon
    )
    print(f"{datetime.now()} Creating Bathy file", flush=True)
    ds_bathy = create_bathy_paleo_orca(topo, custom_orca)
    print(f"{datetime.now()} Creating Soils file", flush=True)
    ds_soils = create_soils(rlat, rlon, omsk)
    print(f"{datetime.now()} Creating High Res file", flush=True)
